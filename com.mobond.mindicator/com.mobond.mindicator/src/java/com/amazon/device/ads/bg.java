/*
 * Decompiled with CFR 0.0.
 * 
 * Could not load the following classes:
 *  com.amazon.device.ads.bp
 *  com.amazon.device.ads.cy
 *  java.io.File
 *  java.lang.Object
 *  java.lang.String
 *  java.lang.StringBuilder
 *  java.lang.System
 */
package com.amazon.device.ads;

import com.amazon.device.ads.bh;
import com.amazon.device.ads.bp;
import com.amazon.device.ads.by;
import com.amazon.device.ads.cv;
import com.amazon.device.ads.cw;
import com.amazon.device.ads.cx;
import com.amazon.device.ads.cy;
import com.amazon.device.ads.cz;
import com.amazon.device.ads.dd;
import com.amazon.device.ads.ee;
import java.io.File;

class bg {
    private static final String a = "bg";
    private static bg b = new bg(cx.a(), (by)new bp(), ee.a(), cv.a());
    private String c;
    private final cx d;
    private final by e;
    private final ee f;
    private final cv g;
    private final cy h = new cz().a(a);

    protected bg(cx cx2, by by2, ee ee2, cv cv2) {
        this.d = cx2;
        this.e = by2;
        this.f = ee2;
        this.g = cv2;
        this.c = this.b();
    }

    public static final bg a() {
        return b;
    }

    private void a(a a2, String string, String string2) {
        this.a(a2, bh.a(string), string2);
    }

    /*
     * Exception decompiling
     */
    private void a(a var1, byte[] var2, String var3) {
        // This method has failed to decompile.  When submitting a bug report, please provide this stack trace, and (if you hold appropriate legal rights) the relevant class file.
        // org.benf.cfr.reader.util.ConfusedCFRException: Invalid stack depths @ lbl40 : ALOAD : trying to set 1 previously set to 0
        // org.benf.cfr.reader.b.a.a.g.a(Op02WithProcessedDataAndRefs.java:203)
        // org.benf.cfr.reader.b.a.a.g.a(Op02WithProcessedDataAndRefs.java:1489)
        // org.benf.cfr.reader.b.f.a(CodeAnalyser.java:308)
        // org.benf.cfr.reader.b.f.a(CodeAnalyser.java:182)
        // org.benf.cfr.reader.b.f.a(CodeAnalyser.java:127)
        // org.benf.cfr.reader.entities.attributes.f.c(AttributeCode.java:96)
        // org.benf.cfr.reader.entities.g.p(Method.java:396)
        // org.benf.cfr.reader.entities.d.e(ClassFile.java:890)
        // org.benf.cfr.reader.entities.d.b(ClassFile.java:792)
        // org.benf.cfr.reader.b.a(Driver.java:128)
        // org.benf.cfr.reader.a.a(CfrDriverImpl.java:63)
        // com.njlabs.showjava.decompilers.JavaExtractionWorker.decompileWithCFR(JavaExtractionWorker.kt:61)
        // com.njlabs.showjava.decompilers.JavaExtractionWorker.doWork(JavaExtractionWorker.kt:130)
        // com.njlabs.showjava.decompilers.BaseDecompiler.withAttempt(BaseDecompiler.kt:108)
        // com.njlabs.showjava.workers.DecompilerWorker$b.run(DecompilerWorker.kt:118)
        // java.util.concurrent.ThreadPoolExecutor.runWorker(ThreadPoolExecutor.java:1167)
        // java.util.concurrent.ThreadPoolExecutor$Worker.run(ThreadPoolExecutor.java:641)
        // java.lang.Thread.run(Thread.java:919)
        throw new IllegalStateException("Decompilation failed");
    }

    private boolean d() {
        a a2 = new a();
        this.a(a2, "", this.a("amazon_ads_close_normal.png"));
        this.a(a2, "", this.a("amazon_ads_close_pressed.png"));
        this.a(a2, "", this.a("amazon_ads_close.png"));
        this.a(a2, "", this.a("amazon_ads_leftarrow.png"));
        this.a(a2, "", this.a("amazon_ads_refresh.png"));
        this.a(a2, "", this.a("amazon_ads_rightarrow.png"));
        this.a(a2, "", this.a("amazon_ads_close_normal.png"));
        this.a(a2, "", this.a("amazon_ads_close_pressed.png"));
        this.a(a2, "", this.a("amazon_ads_open_external_browser.png"));
        return true ^ a2.b();
    }

    public String a(String string) {
        StringBuilder stringBuilder = new StringBuilder();
        stringBuilder.append(this.c);
        stringBuilder.append(string);
        return stringBuilder.toString();
    }

    protected String b() {
        cx cx2 = this.d;
        if (cx2 != null && cx2.j() != null) {
            StringBuilder stringBuilder = new StringBuilder();
            stringBuilder.append(this.d.j().getAbsolutePath());
            stringBuilder.append(File.separator);
            return stringBuilder.toString();
        }
        return null;
    }

    /*
     * Enabled aggressive block sorting
     * Enabled unnecessary exception pruning
     * Enabled aggressive exception aggregation
     */
    public boolean c() {
        bg bg2 = this;
        synchronized (bg2) {
            String string;
            if (this.c == null && (string = (this.c = this.b())) == null) {
                return false;
            }
            long l2 = System.nanoTime();
            int n2 = this.f.a("mraid-assets-version", 0);
            boolean bl2 = false;
            if (n2 < 5) {
                cy cy2 = this.h;
                Object[] arrobject = new Object[]{5};
                cy2.c("Writing assets to disk: version %d", arrobject);
                if (!this.d()) {
                    this.g.b().a(cv.a.n);
                    return false;
                }
                this.f.b("mraid-assets-version", 5);
                bl2 = true;
            }
            long l3 = dd.a(System.nanoTime() - l2);
            cw cw2 = this.g.b();
            cv.a a2 = bl2 ? cv.a.l : cv.a.m;
            cw2.a(a2, l3);
            return true;
        }
    }

    class a {
        boolean a = false;

        a() {
        }

        private void a() {
            this.a = true;
        }

        static /* synthetic */ void b(a a2) {
            a2.a();
        }

        private boolean b() {
            return this.a;
        }
    }

}

